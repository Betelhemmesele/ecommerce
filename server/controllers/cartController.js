...
async function createCart(req, res) {
  try {
    ...
    const cart = await model.Cart.create({ userId, sessionId, token, status: 'active' });
...
async function getCartItems(req, res) {
  try {
    ...
    const cartItems = await model.CartItem.findAll({ where: { cartId } });
    res.json(cartItems);
  } catch (error) {
    res.status(500).json({ error: 'Internal Server Error' });
  }
...
async function updateCartItem(req, res) {
  try {
    ...
    const cartItem = await model.CartItem.findByPk(cartItemId);
    if (!cartItem) {
